...
def water_quality():
    ...
    if is_dataset == 'Yes':
        ...
        if (st.button('Enter',key=str('button'+str(key_counter)),on_click=button_click))|(st.session_state.button_click==True):
            ...
            if determine_percentage=='Enter the data for each parameter manually':
                ...
                for param in params_measured:
                    ...
                    for i in range(total_datapoints):
                        param_values[param].append(get_value(param,count))
                        count+=1
                if st.button('Show percentage of parameters above the limit',key='Show_params'):
                    ...
                    for i in param_values.keys():
                        ...
                        st.write(f'**{param_percentage_above_limit[param]} percentage of recorded samples of {param} are above the acceptable limit.**')
